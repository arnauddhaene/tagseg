...
def cine2gridtag(
    im, label, myo_index: int = 2, mod: float = 0.4, spacing: int = 5, **kwargs
):
    ...
    **kwargs : optional
    ...
    if label is not None:
        ...
        if (
            label.shape != im.shape
        ):  # This may already be checked somewhere, probably not needed
...
def sim_gridtag(
    im, spacing=5, total_flip=70, flip_pattern=(1, 3, 3, 1), x_offset=0, y_offset=0
):
