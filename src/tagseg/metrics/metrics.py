...
def dice_loss(
    prediction: torch.Tensor, target: torch.Tensor, exclude_bg: bool = False
) -> torch.Tensor:
    ...
    return torch.mean(-dice_score(prediction, target)[offset:] + 1.0)
...
def region_loss(
    ...
    rl = (target_one_hot * (1 - input_soft) + (1 - target_one_hot) * input_soft).sum(
        dim=(2, 3)
    )
...
def evaluate(
    ...
    if track_images and not isinstance(run, aim.Run):
        raise ValueError(
            f"Must pass object of type aim.Run to eval function. Passed {type(run)}."
        )
        ...
        for images, targets in tqdm(
            ...
            if track_images:
                ...
                for mask, target in zip(masks, targets.detach().cpu().numpy()):
                    mask, target = (
                        mask.astype("uint8") * factor,
                        target.astype("uint8") * factor,
                    )
                    run.track(
